...
class Server(Thread):
    ...
    def do_login(self, client, check, options):
    ...
    def run(self):
        ...
        with socket(AF_INET, SOCK_STREAM) as s:
            ...
            while self.esci == False:
                ...
                send_credentials(client, LogIn.OPTIONS)
                data = client.recv(BUFFER_SIZE)
                cmd = data.decode('utf8')
                if cmd.upper() == 'L' or cmd.upper() == 'LOGIN':
                   self.do_login(client, check, options) 
                else:
                    send_credentials(client, LogIn.CREDENTIALS)
                    crd_s = receive_credentials_client(client)
                    print(f"Ho ricevuto queste credenziali username {crd_s[0]} e password {crd_s[1]}")
                    if LogIn.do_signUp(crd_s[0], crd_s[1], options[0], options[1]):
                        self.count = 0
                        time.sleep(1)
                        send_credentials(client, LogIn.CONFIRM_SIGNUP)
...
def show_all_user(option, client):
    ...
    elif result == 'Error show all userserver':
        result = 'Error in db'
    else:
